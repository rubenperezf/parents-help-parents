...
const putFamilyReview = async (request, response) => {
    try{
        ...
        var FamilyReviewsInstance = await FamilyReviewsModel.findOneAndUpdate({"_id":request.params.id}, request.body, {useFindAndModify: false})
        console.log(FamilyReviewsInstance);
...
const getFamilyReview = async (request, response) => {
    try{
        ...
        const FamilyReviewsInstance = await FamilyCommentsModel.find({"_id": request.params.id})
        response.send(FamilyReviewsInstance)
    }catch(err){
...
const deleteFamilyReview = async (request, response) => {
    try{
        ...
        const FamilyReviewsInstance = await FamilyReviewsModel.deleteOne({"_id": request.params.id})
        response.send(FamilyReviewsInstance)
    }catch(err){
        response.status(500).send(err)
    }
...
module.exports = {getFamilyReviews, postFamilyReviews, putFamilyReview, getFamilyReview, deleteFamilyReview}