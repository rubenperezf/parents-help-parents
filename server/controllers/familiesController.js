...
const putFamily = async (request, response) => {
    try{
        var FamiliesInstance = await FamiliesModel.findOneAndUpdate({"_id":request.params.id}, request.body, {useFindAndModify: false})
        console.log(FamiliesInstance);
...
const getFamily = async (request, response) => {
    try{
        const FamiliesInstance = await FamiliesModel.find({"_id": request.params.id})
        response.send(FamiliesInstance)
    }catch(err){
...
const deleteFamily = async (request, response) => {
    try{
        const FamiliesInstance = await FamiliesModel.deleteOne({"_id": request.params.id})
        response.send(FamiliesInstance)
    }catch(err){
        response.status(500).send(err)
    }
...
module.exports = {getFamilies, postFamily, putFamily, getFamily, deleteFamily}